...
import { Check, X, Loader2, Shield, ExternalLink, Lock, Pencil, Trash2, Users, UserCog, ShieldCheck, ShieldOff, UserX, Megaphone, Plus, Target, Trophy, BarChart3, Gift, Gift } from "lucide-react";
...
const AdminPanelPage = () => {
  ...
  const [tab, setTab] = useState<"nominations" | "creators" | "users" | "board" | "predictions" | "tournaments" | "rete | "season-rewards"ntion">("nominations");
  ...
  return (
    <div className="min-h-screen bg-background p-3 pb-0">
      <div className="max-w-lg mx-auto space-y-4">
        ...
        {tab === "nominations" ? <NominationsTab /> : tab === "creators" ? <CreatorsTab /> : tab === "users" ? <UsersTab /> : tab === "board" ? <BoardTab /> : tab === "predictions" ? <PredictionsTab /> : tab === "tournaments" ? <TournamentsTab /> : tab === "season-rewards" ? <SeasonRewardsTab /> : <AdminRetentionDashboard />}
      </div>
      <Footer />
    </div>
  );
