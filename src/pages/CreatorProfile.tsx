...
const CreatorProfile = () => {
  ...
  // ─── SNS Links ──────────────────────────────────────────
  const SnsLinks = () => {
    const channelUrl = creator.channel_link || (creator.youtube_channel_id ? `https://youtube.com/channel/${creator.youtube_channel_id}` : "");
    const displayUrl = channelUrl ? channelUrl.replace(/^https?:\/\/(www\.)?/, "").replace(/\/$/, "") : "";
    return (
      <div className="space-y-1.5">
        ...
        {channelUrl && (
          <a href={channelUrl} target="_blank" rel="noopener noreferrer" className="block text-[11px] text-muted-foreground hover:text-primary truncate max-w-[200px] transition-colors">
            {displayUrl}
          </a>
        )}
      </div>
