  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const { data: activeTournaments } = await supabase
        .from("tournaments")
        .select("id, title, round, category, current_round")
      ...
      // Get current round matches for all tournaments
      const allMatches: any[] = [];
      for (const t of activeTournaments) {
        const { data: matches } = await supabase
          .from("tournament_matches")
          .select("id, tournament_id, creator_a_id, creator_b_id, votes_a, votes_b, round, is_completed, status")
          .eq("tournament_id", t.id)
          .eq("round", t.round)
          .or("status.eq.active,is_completed.eq.false")
      ...
      allMatches.forEach((m) => { ids.add(m.creator_a_id); ids.add(m.creator_b_id); });
      ...
      const result: ActiveTournament[] = activeTournaments.map((t) => ({
        id: t.id,
        title: t.title,
        category: t.category || "",
        current_round: t.current_round || "quarterfinal",
        round: t.round,
        matches: allMatches
      ...
      {tournaments.map((t) => (
        <div key={t.id} className="space-y-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-medium text-muted-foreground truncate">{t.title}</span>
            {t.category && (
              <span className="text-[10px] px-1.5 py-0.5 rounded glass-sm text-muted-foreground">{t.category}</span>
            )}
          </div>
